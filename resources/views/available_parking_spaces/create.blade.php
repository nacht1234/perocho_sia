    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 rounded shadow">
                <form method="POST" action="{{ route('available-parking-spaces.store') }}">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 md:gap-x-6 gap-y-6">
                        <div class="mb-4">
                            <label class="block">Building Floor No</label>
                            <input type="text" name="bldg_floor_no" value="{{ old('bldg_floor_no') }}" required class="w-full border px-3 py-2 rounded" />
                            @error('bldg_floor_no') <div class="text-red-600">{{ $message }}</div> @enderror
                        </div>

                        <div class="mb-4">
                            <label class="block">Lot No</label>
                            <input type="text" name="lot_no" value="{{ old('lot_no') }}" required class="w-full border px-3 py-2 rounded" />
                        ...
                        <div class="mb-4">
                            ...
                            <select name="status" class="w-full border px-3 py-2 rounded">
                                ...
                                <option value="booked" @selected(old('status') == 'booked')>Booked</option>
                            </select>
